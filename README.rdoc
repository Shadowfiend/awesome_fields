...
expected. Awesome fields makes this easier to a certain extent, as well. First of
...
used to determine the value for the collection options is +to_param+. If a
...
overridden by providing a collection or object to use instead through the
<tt>:selected</tt> option. If <tt>:collection</tt> is not provided, the class of
this object or of the first item in the list will be used to run the find
metnioned above.
